...
def enforce_auth_and_request_guards():
    ...
    if request.method in ['POST', 'PUT', 'DELETE', 'PATCH']:
        if path == '/login':
            logger.info(
                "AUTH DEBUG: login POST headers origin=%s referer=%s sec_fetch_site=%s has_cookie=%s csrf_form=%s csrf_header=%s",
...
db_url = os.environ.get('DATABASE_URL', 'postgresql://postgres:prdeuXwtBzpLZaOGpxgRspfjfLNEQrys@gondola.proxy.rlwy.net:25504/railway')
